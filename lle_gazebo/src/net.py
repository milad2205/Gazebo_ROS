...
class PolicyGradient:
    ...
    def choose_action(self, observation):
        ...
        #按照给定的概率采样
        action1 = np.random.choice(range(prob_weights.shape[1]), p=prob_weights.ravel())
        action2 = np.random.choice(range(prob_weights.shape[1]), p=np.ones(9)/ 9)
        action = np.random.choice(np.array([action1,action2]), p=np.array([0.7,0.3]))
        return action1
